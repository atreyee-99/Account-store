...
class entity_add:
    ...
    #Store in file after encrypting the records
    def write(self):
        f = open('emails.txt', "a")
        n = self.name
        e = self.email
...
class entity_display:
    ...
    #Display after deccrypting the records
...
#Function for the event of button click
def onsubmit():
    m = email.get()
    p = password.get()
    n = name.get()
    if len(m)==0 or len(p)==0 or len(n)==0 :
        return
...
#Clears the existing records in file
def clearfile():
    f = open('emails.txt', "w")
    f.close()
...
#Creating window labels and entries
...
submit = Button(window, text='Add Email', command=onsubmit, font=('Courier', 14), bg='lightblue')
...
#Setting the positions of labels
